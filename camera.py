...
def pegar_objeto():
    """Executa sequência automática de pega."""
    global pegando, ultima_pega
    
    if pegando or (time.time() - ultima_pega) < 5:
        return
    
    pegando = True
    print("\n🤖 INICIANDO PEGA DO OBJETO!")
    
    try:
        print("  1. Abrindo garra...")
        IA.executar_acao("ABRIR_GARRA")
        time.sleep(1.5)
        
        print("  2. Abaixando...")
        IA.executar_acao("ABAIXAR")
        time.sleep(1.5)
        
        print("  3. Fechando garra...")
        IA.executar_acao("FECHAR_GARRA")
        time.sleep(1.5)
        
        print("  4. Levantando objeto...")
        IA.executar_acao("LEVANTAR")
        time.sleep(1.5)
        
        print("✅ OBJETO CAPTURADO COM SUCESSO!")
    except Exception as e:
        print(f"❌ Erro na pega: {e}")
    finally:
        pegando = False
        ultima_pega = time.time()
...
while True:
    ...
    for cnt in contours:
        ...
        if area > 1000:
            ...
            # Se objeto está muito perto e centralizado, pega automaticamente
            if area > 20000 and 320 - 80 < centro_x < 320 + 80 and 240 - 80 < centro_y < 240 + 80:
                print("📍 OBJETO CENTRALIZADO E PRÓXIMO! TENTANDO PEGAR...")
                pegar_objeto()
